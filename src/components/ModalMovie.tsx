...
function Modal({ data, category, type }: IModal) {
  ...
  const clickedMovie =
    ...
    data?.results.find(
      (movie: ISliderCommon) => String(movie.id) === movieMatch?.params.movieId
    );

  const { data: detail } = useQuery<IMovieDetail>(
    ["details", `detail${movieMatch?.params.movieId}`],
    () => getMovieDetail(movieMatch?.params.movieId)
  );
  const { data: credit } = useQuery<ICredit>(
    ["credits", `credit${movieMatch?.params.movieId}`],
    () => getCredits(movieMatch?.params.movieId)
  );

  const { data: similar } = useQuery<IResults>(
    ["similars", `similar${movieMatch?.params.movieId}`],
    () => getSimilar(movieMatch?.params.movieId)
  );
      ...
      {movieMatch ?
        <>
          ...
          <ModalWrap
            ...
            layoutId={`${movieMatch.params.movieId}${category}${type}`}
          >
            {clickedMovie && detail ? <>
              ...
              <ModalTextInfo>
                <TextInfoLeft>
                  <ModalBedge>
                    ...
                    <span>{detail ? `${hour}hr ${minutes}min` : "-"}</span>
                    <span>
